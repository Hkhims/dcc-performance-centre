-- Harden match-import approval cross-record consistency.
--
-- The approval boundary must only publish DCC teams attached to the external
-- match, and every player performance must belong to the exact published team
-- entry identified by source_match_id + team_id.
--
-- This function retains the existing mandatory match-date protection.

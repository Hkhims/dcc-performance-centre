-- Require every approved canonical match to have a valid match date.
--
-- This hardens the publication boundary used by both the NV Play importer
-- and the future DCC mobile app scorer. Canonical match reconciliation relies
-- on team + date + competition, so a match without a date must never be
-- published into the canonical DCC tables.
